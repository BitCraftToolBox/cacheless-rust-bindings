...
///
/// Obtain a handle from the [`ExtractionRecipeDescTableAccess::extraction_recipe_desc`] method on [`super::RemoteTables`],
/// like `ctx.db.extraction_recipe_desc()`.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.extraction_recipe_desc().on_insert(...)`.
...
pub trait ExtractionRecipeDescTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`ExtractionRecipeDescTableHandle`], which mediates access to the table `extraction_recipe_desc`.
