// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{
	self as __sdk,
	__lib,
	__sats,
	__ws,
};
use super::pathfinding_desc_type::PathfindingDesc;
use super::pathfinding_traversal_option_type::PathfindingTraversalOption;

/// Table handle for the table `pathfinding_desc`.
///
/// Obtain a handle from the [`PathfindingDescTableAccess::pathfinding_desc`] method on [`super::RemoteTables`],
/// like `ctx.db.pathfinding_desc()`.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.pathfinding_desc().on_insert(...)`.
pub struct PathfindingDescTableHandle<'ctx> {
    imp: __sdk::TableHandle<PathfindingDesc>,
    ctx: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

/// Lifetime-aware accessor marker for the table `pathfinding_desc`.
pub struct PathfindingDescTableAccessor;

impl __sdk::TableAccessor<super::RemoteTables> for PathfindingDescTableAccessor {
    type Row = PathfindingDesc;
    type Handle<'db> = PathfindingDescTableHandle<'db>;

    fn get<'db>(db: &'db super::RemoteTables) -> Self::Handle<'db> {
        db.pathfinding_desc()
    }
}

#[allow(non_camel_case_types)]
/// Extension trait for access to the table `pathfinding_desc`.
///
/// Implemented for [`super::RemoteTables`].
pub trait PathfindingDescTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`PathfindingDescTableHandle`], which mediates access to the table `pathfinding_desc`.
    fn pathfinding_desc(&self) -> PathfindingDescTableHandle<'_>;
}

impl PathfindingDescTableAccess for super::RemoteTables {
    fn pathfinding_desc(&self) -> PathfindingDescTableHandle<'_> {
        PathfindingDescTableHandle {
            imp: self.imp.get_table::<PathfindingDesc>("pathfinding_desc"),
            ctx: std::marker::PhantomData,
        }
    }
}

pub struct PathfindingDescInsertCallbackId(__sdk::CallbackId);
pub struct PathfindingDescDeleteCallbackId(__sdk::CallbackId);

impl<'ctx> __sdk::TableLike for PathfindingDescTableHandle<'ctx> {
    type Row = PathfindingDesc;
    type EventContext = super::EventContext;

    fn count(&self) -> u64 { self.imp.count() }
    fn iter(&self) -> impl Iterator<Item = PathfindingDesc> + '_ { self.imp.iter() }
}

impl<'ctx> __sdk::Table for PathfindingDescTableHandle<'ctx> {
    type Row = PathfindingDesc;
    type EventContext = super::EventContext;

    fn count(&self) -> u64 { self.imp.count() }
    fn iter(&self) -> impl Iterator<Item = PathfindingDesc> + '_ { self.imp.iter() }

    type InsertCallbackId = PathfindingDescInsertCallbackId;

    fn on_insert(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> PathfindingDescInsertCallbackId {
        PathfindingDescInsertCallbackId(self.imp.on_insert(Box::new(callback)))
    }

    fn remove_on_insert(&self, callback: PathfindingDescInsertCallbackId) {
        self.imp.remove_on_insert(callback.0)
    }

    type DeleteCallbackId = PathfindingDescDeleteCallbackId;

    fn on_delete(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> PathfindingDescDeleteCallbackId {
        PathfindingDescDeleteCallbackId(self.imp.on_delete(Box::new(callback)))
    }

    fn remove_on_delete(&self, callback: PathfindingDescDeleteCallbackId) {
        self.imp.remove_on_delete(callback.0)
    }
}

impl<'ctx> __sdk::WithInsert for PathfindingDescTableHandle<'ctx> {
    type InsertCallbackId = PathfindingDescInsertCallbackId;

    fn on_insert(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> PathfindingDescInsertCallbackId {
        PathfindingDescInsertCallbackId(self.imp.on_insert(Box::new(callback)))
    }

    fn remove_on_insert(&self, callback: PathfindingDescInsertCallbackId) {
        self.imp.remove_on_insert(callback.0)
    }
}

impl<'ctx> __sdk::WithDelete for PathfindingDescTableHandle<'ctx> {
    type DeleteCallbackId = PathfindingDescDeleteCallbackId;

    fn on_delete(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> PathfindingDescDeleteCallbackId {
        PathfindingDescDeleteCallbackId(self.imp.on_delete(Box::new(callback)))
    }

    fn remove_on_delete(&self, callback: PathfindingDescDeleteCallbackId) {
        self.imp.remove_on_delete(callback.0)
    }
}

pub struct PathfindingDescUpdateCallbackId(__sdk::CallbackId);

impl<'ctx> __sdk::TableWithPrimaryKey for PathfindingDescTableHandle<'ctx> {
    type UpdateCallbackId = PathfindingDescUpdateCallbackId;

    fn on_update(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row, &Self::Row) + Send + 'static,
    ) -> PathfindingDescUpdateCallbackId {
        PathfindingDescUpdateCallbackId(self.imp.on_update(Box::new(callback)))
    }

    fn remove_on_update(&self, callback: PathfindingDescUpdateCallbackId) {
        self.imp.remove_on_update(callback.0)
    }
}

impl<'ctx> __sdk::WithUpdate for PathfindingDescTableHandle<'ctx> {
    type UpdateCallbackId = PathfindingDescUpdateCallbackId;

    fn on_update(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row, &Self::Row) + Send + 'static,
    ) -> PathfindingDescUpdateCallbackId {
        PathfindingDescUpdateCallbackId(self.imp.on_update(Box::new(callback)))
    }

    fn remove_on_update(&self, callback: PathfindingDescUpdateCallbackId) {
        self.imp.remove_on_update(callback.0)
    }
}

        /// Access to the `id` unique index on the table `pathfinding_desc`,
        /// which allows point queries on the field of the same name
        /// via the [`PathfindingDescIdUnique::find`] method.
        ///
        /// Users are encouraged not to explicitly reference this type,
        /// but to directly chain method calls,
        /// like `ctx.db.pathfinding_desc().id().find(...)`.
        pub struct PathfindingDescIdUnique<'ctx> {
            imp: __sdk::UniqueConstraintHandle<PathfindingDesc, i32>,
            phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
        }

        impl<'ctx> PathfindingDescTableHandle<'ctx> {
            /// Get a handle on the `id` unique index on the table `pathfinding_desc`.
            pub fn id(&self) -> PathfindingDescIdUnique<'ctx> {
                PathfindingDescIdUnique {
                    imp: self.imp.get_unique_constraint::<i32>("id"),
                    phantom: std::marker::PhantomData,
                }
            }
        }

        impl<'ctx> PathfindingDescIdUnique<'ctx> {
            /// Find the subscribed row whose `id` column value is equal to `col_val`,
            /// if such a row is present in the client cache.
            pub fn find(&self, col_val: &i32) -> Option<PathfindingDesc> {
                self.imp.find(col_val)
            }
        }
        
#[doc(hidden)]
pub(super) fn register_table(client_cache: &mut __sdk::ClientCache<super::RemoteModule>) {

    let _table = client_cache.get_or_make_table::<PathfindingDesc>("pathfinding_desc");
    _table.add_unique_constraint::<i32>("id", |row| &row.id);
}

#[doc(hidden)]
pub(super) fn parse_table_update(
    raw_updates: __ws::v2::TableUpdate,
) -> __sdk::Result<__sdk::TableUpdate<PathfindingDesc>> {
    __sdk::TableUpdate::parse_table_update(raw_updates).map_err(|e| {
        __sdk::InternalError::failed_parse(
            "TableUpdate<PathfindingDesc>",
            "TableUpdate",
        ).with_cause(e).into()
    })
}

        #[allow(non_camel_case_types)]
        /// Extension trait for query builder access to the table `PathfindingDesc`.
        ///
        /// Implemented for [`__sdk::QueryTableAccessor`].
        pub trait pathfinding_descQueryTableAccess {
            #[allow(non_snake_case)]
            /// Get a query builder for the table `PathfindingDesc`.
            fn pathfinding_desc(&self) -> __sdk::__query_builder::Table<PathfindingDesc>;
        }

        impl pathfinding_descQueryTableAccess for __sdk::QueryTableAccessor {
            fn pathfinding_desc(&self) -> __sdk::__query_builder::Table<PathfindingDesc> {
                __sdk::__query_builder::Table::new("pathfinding_desc")
            }
        }

