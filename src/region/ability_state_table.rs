...
///
/// Obtain a handle from the [`AbilityStateTableAccess::ability_state`] method on [`super::RemoteTables`],
/// like `ctx.db.ability_state()`.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.ability_state().on_insert(...)`.
...
/// Extension trait for access to the table `ability_state`.
///
/// Implemented for [`super::RemoteTables`].
pub trait AbilityStateTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`AbilityStateTableHandle`], which mediates access to the table `ability_state`.
    fn ability_state(&self) -> AbilityStateTableHandle<'_>;
}

impl AbilityStateTableAccess for super::RemoteTables {
    fn ability_state(&self) -> AbilityStateTableHandle<'_> {
        AbilityStateTableHandle {
            imp: self.imp.get_table::<AbilityState>("ability_state"),
