...
///
/// Obtain a handle from the [`PlaceableGrowthDescTableAccess::placeable_growth_desc`] method on [`super::RemoteTables`],
/// like `ctx.db.placeable_growth_desc()`.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.placeable_growth_desc().on_insert(...)`.
...
pub trait PlaceableGrowthDescTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`PlaceableGrowthDescTableHandle`], which mediates access to the table `placeable_growth_desc`.
