...
///
/// Obtain a handle from the [`BiomeDescTableAccess::biome_desc`] method on [`super::RemoteTables`],
/// like `ctx.db.biome_desc()`.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.biome_desc().on_insert(...)`.
...
/// Extension trait for access to the table `biome_desc`.
///
/// Implemented for [`super::RemoteTables`].
pub trait BiomeDescTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`BiomeDescTableHandle`], which mediates access to the table `biome_desc`.
    fn biome_desc(&self) -> BiomeDescTableHandle<'_>;
}

impl BiomeDescTableAccess for super::RemoteTables {
    fn biome_desc(&self) -> BiomeDescTableHandle<'_> {
        BiomeDescTableHandle {
            imp: self.imp.get_table::<BiomeDesc>("biome_desc"),
