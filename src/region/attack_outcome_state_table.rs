...
///
/// Obtain a handle from the [`AttackOutcomeStateTableAccess::attack_outcome_state`] method on [`super::RemoteTables`],
/// like `ctx.db.attack_outcome_state()`.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.attack_outcome_state().on_insert(...)`.
...
pub trait AttackOutcomeStateTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`AttackOutcomeStateTableHandle`], which mediates access to the table `attack_outcome_state`.
