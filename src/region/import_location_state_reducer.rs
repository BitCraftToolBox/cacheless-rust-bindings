...
pub trait import_location_state {
    ...
    /// /// Use [`import_location_state:import_location_state_then`] to run a callback after the reducer completes.
    fn import_location_state(&self, records: Vec::<LocationState>,
) -> __sdk::Result<()> {
        self.import_location_state_then(records,  |_, _| {})
    }
...
impl import_location_state for super::RemoteReducers {
    fn import_location_state_then(
        ...
        self.imp.invoke_reducer_with_callback(ImportLocationStateArgs { records,  }, callback)
    }
}
