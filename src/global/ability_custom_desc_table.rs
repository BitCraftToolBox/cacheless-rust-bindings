...
///
/// Obtain a handle from the [`AbilityCustomDescTableAccess::ability_custom_desc`] method on [`super::RemoteTables`],
/// like `ctx.db.ability_custom_desc()`.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.ability_custom_desc().on_insert(...)`.
pub struct AbilityCustomDescTableHandle<'ctx> {
    imp: __sdk::TableHandle<AbilityCustomDesc>,
...
pub trait AbilityCustomDescTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`AbilityCustomDescTableHandle`], which mediates access to the table `ability_custom_desc`.
    fn ability_custom_desc(&self) -> AbilityCustomDescTableHandle<'_>;
}

impl AbilityCustomDescTableAccess for super::RemoteTables {
    fn ability_custom_desc(&self) -> AbilityCustomDescTableHandle<'_> {
        AbilityCustomDescTableHandle {
            imp: self.imp.get_table::<AbilityCustomDesc>("ability_custom_desc"),
...
impl<'ctx> __sdk::TableLike for AbilityCustomDescTableHandle<'ctx> {
    ...
    fn iter(&self) -> impl Iterator<Item = AbilityCustomDesc> + '_ { self.imp.iter() }
}
...
impl<'ctx> __sdk::WithUpdate for AbilityCustomDescTableHandle<'ctx> {
    ...
    fn remove_on_update(&self, callback: AbilityCustomDescUpdateCallbackId) {
        ...
        /// Users are encouraged not to explicitly reference this type,
        /// but to directly chain method calls,
        /// like `ctx.db.ability_custom_desc().id().find(...)`.
        pub struct AbilityCustomDescIdUnique<'ctx> {
            imp: __sdk::UniqueConstraintHandle<AbilityCustomDesc, i32>,
            phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
        }

        impl<'ctx> AbilityCustomDescTableHandle<'ctx> {
            /// Get a handle on the `id` unique index on the table `ability_custom_desc`.
            pub fn id(&self) -> AbilityCustomDescIdUnique<'ctx> {
                AbilityCustomDescIdUnique {
        ...
        impl<'ctx> AbilityCustomDescIdUnique<'ctx> {
