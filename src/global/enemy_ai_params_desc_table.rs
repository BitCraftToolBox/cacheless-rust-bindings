...
///
/// Obtain a handle from the [`EnemyAiParamsDescTableAccess::enemy_ai_params_desc`] method on [`super::RemoteTables`],
/// like `ctx.db.enemy_ai_params_desc()`.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.enemy_ai_params_desc().on_insert(...)`.
...
pub trait EnemyAiParamsDescTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`EnemyAiParamsDescTableHandle`], which mediates access to the table `enemy_ai_params_desc`.
