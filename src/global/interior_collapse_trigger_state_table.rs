// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{
	self as __sdk,
	__lib,
	__sats,
	__ws,
};
use super::interior_collapse_trigger_state_type::InteriorCollapseTriggerState;

/// Table handle for the table `interior_collapse_trigger_state`.
///
/// Obtain a handle from the [`InteriorCollapseTriggerStateTableAccess::interior_collapse_trigger_state`] method on [`super::RemoteTables`],
/// like `ctx.db.interior_collapse_trigger_state()`.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.interior_collapse_trigger_state().on_insert(...)`.
pub struct InteriorCollapseTriggerStateTableHandle<'ctx> {
    imp: __sdk::TableHandle<InteriorCollapseTriggerState>,
    ctx: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

/// Lifetime-aware accessor marker for the table `interior_collapse_trigger_state`.
pub struct InteriorCollapseTriggerStateTableAccessor;

impl __sdk::TableAccessor<super::RemoteTables> for InteriorCollapseTriggerStateTableAccessor {
    type Row = InteriorCollapseTriggerState;
    type Handle<'db> = InteriorCollapseTriggerStateTableHandle<'db>;

    fn get<'db>(db: &'db super::RemoteTables) -> Self::Handle<'db> {
        db.interior_collapse_trigger_state()
    }
}

#[allow(non_camel_case_types)]
/// Extension trait for access to the table `interior_collapse_trigger_state`.
///
/// Implemented for [`super::RemoteTables`].
pub trait InteriorCollapseTriggerStateTableAccess {
    #[allow(non_snake_case)]
    /// Obtain a [`InteriorCollapseTriggerStateTableHandle`], which mediates access to the table `interior_collapse_trigger_state`.
    fn interior_collapse_trigger_state(&self) -> InteriorCollapseTriggerStateTableHandle<'_>;
}

impl InteriorCollapseTriggerStateTableAccess for super::RemoteTables {
    fn interior_collapse_trigger_state(&self) -> InteriorCollapseTriggerStateTableHandle<'_> {
        InteriorCollapseTriggerStateTableHandle {
            imp: self.imp.get_table::<InteriorCollapseTriggerState>("interior_collapse_trigger_state"),
            ctx: std::marker::PhantomData,
        }
    }
}

pub struct InteriorCollapseTriggerStateInsertCallbackId(__sdk::CallbackId);
pub struct InteriorCollapseTriggerStateDeleteCallbackId(__sdk::CallbackId);

impl<'ctx> __sdk::TableLike for InteriorCollapseTriggerStateTableHandle<'ctx> {
    type Row = InteriorCollapseTriggerState;
    type EventContext = super::EventContext;

    fn count(&self) -> u64 { self.imp.count() }
    fn iter(&self) -> impl Iterator<Item = InteriorCollapseTriggerState> + '_ { self.imp.iter() }
}

impl<'ctx> __sdk::Table for InteriorCollapseTriggerStateTableHandle<'ctx> {
    type Row = InteriorCollapseTriggerState;
    type EventContext = super::EventContext;

    fn count(&self) -> u64 { self.imp.count() }
    fn iter(&self) -> impl Iterator<Item = InteriorCollapseTriggerState> + '_ { self.imp.iter() }

    type InsertCallbackId = InteriorCollapseTriggerStateInsertCallbackId;

    fn on_insert(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> InteriorCollapseTriggerStateInsertCallbackId {
        InteriorCollapseTriggerStateInsertCallbackId(self.imp.on_insert(Box::new(callback)))
    }

    fn remove_on_insert(&self, callback: InteriorCollapseTriggerStateInsertCallbackId) {
        self.imp.remove_on_insert(callback.0)
    }

    type DeleteCallbackId = InteriorCollapseTriggerStateDeleteCallbackId;

    fn on_delete(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> InteriorCollapseTriggerStateDeleteCallbackId {
        InteriorCollapseTriggerStateDeleteCallbackId(self.imp.on_delete(Box::new(callback)))
    }

    fn remove_on_delete(&self, callback: InteriorCollapseTriggerStateDeleteCallbackId) {
        self.imp.remove_on_delete(callback.0)
    }
}

impl<'ctx> __sdk::WithInsert for InteriorCollapseTriggerStateTableHandle<'ctx> {
    type InsertCallbackId = InteriorCollapseTriggerStateInsertCallbackId;

    fn on_insert(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> InteriorCollapseTriggerStateInsertCallbackId {
        InteriorCollapseTriggerStateInsertCallbackId(self.imp.on_insert(Box::new(callback)))
    }

    fn remove_on_insert(&self, callback: InteriorCollapseTriggerStateInsertCallbackId) {
        self.imp.remove_on_insert(callback.0)
    }
}

impl<'ctx> __sdk::WithDelete for InteriorCollapseTriggerStateTableHandle<'ctx> {
    type DeleteCallbackId = InteriorCollapseTriggerStateDeleteCallbackId;

    fn on_delete(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row) + Send + 'static,
    ) -> InteriorCollapseTriggerStateDeleteCallbackId {
        InteriorCollapseTriggerStateDeleteCallbackId(self.imp.on_delete(Box::new(callback)))
    }

    fn remove_on_delete(&self, callback: InteriorCollapseTriggerStateDeleteCallbackId) {
        self.imp.remove_on_delete(callback.0)
    }
}

pub struct InteriorCollapseTriggerStateUpdateCallbackId(__sdk::CallbackId);

impl<'ctx> __sdk::TableWithPrimaryKey for InteriorCollapseTriggerStateTableHandle<'ctx> {
    type UpdateCallbackId = InteriorCollapseTriggerStateUpdateCallbackId;

    fn on_update(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row, &Self::Row) + Send + 'static,
    ) -> InteriorCollapseTriggerStateUpdateCallbackId {
        InteriorCollapseTriggerStateUpdateCallbackId(self.imp.on_update(Box::new(callback)))
    }

    fn remove_on_update(&self, callback: InteriorCollapseTriggerStateUpdateCallbackId) {
        self.imp.remove_on_update(callback.0)
    }
}

impl<'ctx> __sdk::WithUpdate for InteriorCollapseTriggerStateTableHandle<'ctx> {
    type UpdateCallbackId = InteriorCollapseTriggerStateUpdateCallbackId;

    fn on_update(
        &self,
        callback: impl FnMut(&Self::EventContext, &Self::Row, &Self::Row) + Send + 'static,
    ) -> InteriorCollapseTriggerStateUpdateCallbackId {
        InteriorCollapseTriggerStateUpdateCallbackId(self.imp.on_update(Box::new(callback)))
    }

    fn remove_on_update(&self, callback: InteriorCollapseTriggerStateUpdateCallbackId) {
        self.imp.remove_on_update(callback.0)
    }
}

        /// Access to the `entity_id` unique index on the table `interior_collapse_trigger_state`,
        /// which allows point queries on the field of the same name
        /// via the [`InteriorCollapseTriggerStateEntityIdUnique::find`] method.
        ///
        /// Users are encouraged not to explicitly reference this type,
        /// but to directly chain method calls,
        /// like `ctx.db.interior_collapse_trigger_state().entity_id().find(...)`.
        pub struct InteriorCollapseTriggerStateEntityIdUnique<'ctx> {
            imp: __sdk::UniqueConstraintHandle<InteriorCollapseTriggerState, u64>,
            phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
        }

        impl<'ctx> InteriorCollapseTriggerStateTableHandle<'ctx> {
            /// Get a handle on the `entity_id` unique index on the table `interior_collapse_trigger_state`.
            pub fn entity_id(&self) -> InteriorCollapseTriggerStateEntityIdUnique<'ctx> {
                InteriorCollapseTriggerStateEntityIdUnique {
                    imp: self.imp.get_unique_constraint::<u64>("entity_id"),
                    phantom: std::marker::PhantomData,
                }
            }
        }

        impl<'ctx> InteriorCollapseTriggerStateEntityIdUnique<'ctx> {
            /// Find the subscribed row whose `entity_id` column value is equal to `col_val`,
            /// if such a row is present in the client cache.
            pub fn find(&self, col_val: &u64) -> Option<InteriorCollapseTriggerState> {
                self.imp.find(col_val)
            }
        }
        
#[doc(hidden)]
pub(super) fn register_table(client_cache: &mut __sdk::ClientCache<super::RemoteModule>) {

    let _table = client_cache.get_or_make_table::<InteriorCollapseTriggerState>("interior_collapse_trigger_state");
    _table.add_unique_constraint::<u64>("entity_id", |row| &row.entity_id);
}

#[doc(hidden)]
pub(super) fn parse_table_update(
    raw_updates: __ws::v2::TableUpdate,
) -> __sdk::Result<__sdk::TableUpdate<InteriorCollapseTriggerState>> {
    __sdk::TableUpdate::parse_table_update(raw_updates).map_err(|e| {
        __sdk::InternalError::failed_parse(
            "TableUpdate<InteriorCollapseTriggerState>",
            "TableUpdate",
        ).with_cause(e).into()
    })
}

        #[allow(non_camel_case_types)]
        /// Extension trait for query builder access to the table `InteriorCollapseTriggerState`.
        ///
        /// Implemented for [`__sdk::QueryTableAccessor`].
        pub trait interior_collapse_trigger_stateQueryTableAccess {
            #[allow(non_snake_case)]
            /// Get a query builder for the table `InteriorCollapseTriggerState`.
            fn interior_collapse_trigger_state(&self) -> __sdk::__query_builder::Table<InteriorCollapseTriggerState>;
        }

        impl interior_collapse_trigger_stateQueryTableAccess for __sdk::QueryTableAccessor {
            fn interior_collapse_trigger_state(&self) -> __sdk::__query_builder::Table<InteriorCollapseTriggerState> {
                __sdk::__query_builder::Table::new("interior_collapse_trigger_state")
            }
        }

